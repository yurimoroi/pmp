...
const COMMON_CLASSES = {
  tableCell: "py-4 px-6",
  tableHeader: "py-4 px-6 text-left text-sm font-bold text-gray-700 min-w-[120px]",
            ...
            annualUpdateChildren.map((child) => (
              <tr key={child.childId} className={`border-t border-gray-100 ${COMMON_CLASSES.rowHover}`}>
                <td className={COMMON_CLASSES.tableCell}>
                  <span className="text-gray-700">{child.childId}</span>
                </td>
                <td className={COMMON_CLASSES.tableCell}>
                  <span className={COMMON_CLASSES.text}>{child.name}</span>
                </td>
                <td className={COMMON_CLASSES.tableCell}>
                  <span className={COMMON_CLASSES.text}>{child.className}</span>
                </td>
                <td className={COMMON_CLASSES.tableCell}>
                  <select
                    className={`w-full ${COMMON_CLASSES.text}`}
                ...
                <td className={COMMON_CLASSES.tableCell}>
                  <span className={COMMON_CLASSES.text}>{format(new Date(child.admissionAt), "yyyy/MM/dd")}</span>
                </td>
                <td className={COMMON_CLASSES.tableCell}>
                  <span className={COMMON_CLASSES.text}>{child.leavingAt ? format(new Date(child.leavingAt), "yyyy/MM/dd") : "-"}</span>
                </td>
              </tr>
            ))
          )}
